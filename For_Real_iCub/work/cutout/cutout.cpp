...
int main(int argc, char *argv[]){
  ...
  printf("%s\n",trialname);
  ppm_read("/home/icub/Desktop/Akira/iCub/datadump/initial/leftcam.ppm", back); //背景ファイルの読み込み
  ...
  printf("file open:%s\n",filename); 
  strcpy(trialname,argv[1]);
  sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/object_cam_point.txt",trialname);//
  ...
  //sprintf(trialname,argv[1]);// = argv[1];
  strcpy(trialname,argv[1]);
  sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/cam/left/00000001.ppm",trialname);
  ...
  while (no == 1){
    strcpy(trialname,argv[1]);    
    sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/cam/left/0000000%d.ppm",trialname,i);
    printf("read:%s\n",filename);  
    no = ppm_read(filename, image); //ファイルの読み込み
    readok = i;
    i++;
  ...
  for(y=0;y<240;y++){ //単純な差分
    for(x=0;x<320;x++){
      ...
      if (x < 70 || y < 55 || x > 310 || y > 225){
        diff = 0;
      }
      if(diff>20000) *(sabun+320*y+x) = 0xff;
  ...
  sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/image/sabun.pgm",trialname);
  printf("write:%s\n",filename);  
  ...
  /*** 1,新規画像読み込み ***/
  strcpy(trialname,argv[1]);    
  sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/cam/left/0000000%d.ppm",trialname,readok);
  printf("read:%s\n",filename); 
  ...
  /*** 3,画像保存 ***/
  strcpy(trialname,argv[1]);
  sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/image/mask.ppm",trialname);
  printf("write:%s\n",filename);  
  ...
  */
  ...
  //sprintf(trialname,argv[1]);// = argv[1];
  strcpy(trialname,argv[1]);
  sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/image/label.ppm",trialname);//
  printf("write:%s\n",filename);
  ...
  for (int i = 0;i< object_num;i++){
    if(i >= 4){
      ...
      //sprintf(trialname,argv[1]);// = argv[1];
      strcpy(trialname,argv[1]);
      sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/image/object_%02d.ppm",trialname,i+1);//
      printf("write:%s\n",filename);
      ::cvSaveImage(filename, ipl);
      
      strcpy(trialname,argv[1]);
      sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/image/obj_mask_%02d.ppm",trialname,i+1);//
      printf("write:%s\n",filename);
      ...
      //cv::Point2f pts1[] = {cv::Point2f(102,54),cv::Point2f(-16,240),cv::Point2f(408,240),cv::Point2f(263,54)};
      ...
      strcpy(trialname,argv[1]);
      sprintf(filename,"/home/icub/Desktop/Akira/iCub/datadump/%s/image/henkan_%02d.pgm",trialname,i+1);//
      ...
      if(0 <= x && x <= 200 && 0 <= y && y <= 200 && 0 <= objx[i] && objx[i] <= 200 && 0 <= objy[i] && objy[i] <= 200){
         xd[0]= (objy[i]/200.0)*0.57 - 0.785;//+ 0.026;//-0.3;//-0.16;
         xd[1]= (objx[i]/200.0)*0.57 - 0.30;//0.295;// + 0.1;// - 0.10;//0.0;//0.6;
         xd[2]= 0.085;// - 0.1; //0.579 - 0.5976;//0.1;//0.23;
         fprintf(fp,"%d, %f,%f,%f\n",i+1,xd[0],xd[1],xd[2]);
	  }
	  else{

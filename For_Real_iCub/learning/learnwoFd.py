...
# Basically, Gibbs sampling of GMMs and LDA (Fdの制約なし)
# Akira Taniguchi 2016/05/31-2016/6/15-2017/03/15
##############################################
    ...
    for d in xrange(D):
      ...
      for m in xrange(M[d]):
        ...
        if DATAnum == 0:
          ...
          for line in open(foldername+str(d+sn).zfill(3)+'/image/object_RGB_BoF_'+str(D)+'_'+str(m+1).zfill(2)+'.csv', 'r'):
            itemList = line[:-1].split(',')
            #print c
            #W_index = W_index + [itemList]
            for i in xrange(len(itemList)):
...
def simulate(foldername,trialname,filename,sn,en, M, N, w_dn, a_d, p_dm, o_dm, c_dm, Ad):
      ...
      Fd = [ Sample_Frame_Random(N[d]) for d in xrange(D)] #初期値はランダムに設定("a","p","o","c")
      ...
      for iter in xrange(num_iter):   #イテレーションを行う
        ...
        if nonpara == 0:
          ...
          temp = np.array([cc[k] + (alpha_p / float(Kp)) for k in xrange(Kp)])
        ...
        if nonpara == 0:
          ...
          temp = np.array([cc[k] + (alpha_o / float(Ko)) for k in xrange(Ko)]) 
        ...
        if nonpara == 0:
          ...
          temp = np.array([cc[k] + (alpha_c / float(Kc)) for k in xrange(Kc)])
        ...
        for d in xrange(D):
          #print modality,N[d]
          #print itertools.product(modality,repeat=N[d])
          F_temp = [f for f in itertools.product(modality,repeat=N[d])]  # デカルト積 繰り返しを許す: 1,1 がある 順序が違えば別と見なす: 1,2 と 2,1 は別
...
if __name__ == '__main__':
    ...
    end   = str(D) #raw_input("end number?>")

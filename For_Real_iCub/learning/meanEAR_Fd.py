#coding:utf-8
#ARI
#Akira Taniguchi 2016/09/21-2016/12/12(ts datanumber input)
#!/usr/bin/env python
import sys
import string
...
trialname = "ts"
tsstart = "1"#raw_input("start number?>")
tsend   = raw_input("ts end number?>")
tssn = int(tsstart)
tsen = int(tsend)
tsData = int(tsen) - int(tssn) +1
trial_se = "("+str(tssn).zfill(3)+"-"+str(tsen).zfill(3)+")"#"(001-040)"#raw_input("trialname?(folder) >")
...
#ARI_Fd_M = [[] for c in xrange(Data)]
EAR_Fd_M = [[] for c in xrange(Data)]
#PARs_M = [[] for c in xrange(N)]
#PARw_M = [[] for c in xrange(N)]
#MM = [ np.array([[] for m in xrange(10) ]) for n in xrange(N)]
#MM_M = 

#fp = open(foldername + '/' + filename1 + str(sn).zfill(3) + "-" + str(en).zfill(3) + filename2 + '_avgARI_Fd.csv', 'w')
fp = open(foldername + '/' + filename1 + str(sn).zfill(3) + "-" + str(en).zfill(3) + filename2 + '_avgEAR_Fd.csv', 'w')
...
for s in range(Data):
  ...
  for line in open(foldername +'/' + filename + '/' + trialname + '_'+ filename +'_EAR_Fd.csv', 'r'):
    itemList = line[:-1].split(',')
    if (i == 1) and (itemList[0] != ''):
      #print i,itemList
      #MI_M[s] = MI_M[s] + [float(itemList[0])]
      EAR_Fd_M[s] = EAR_Fd_M[s] + [float(itemList[0])]
  ...
  EAR_Fd_M[s] = np.array(EAR_Fd_M[s])
...
fp.write('EAR_Fd\n')
fp.write(repr(EAR_Fd_MM[0])+'\n')
fp.write( str(np.std(EAR_Fd_M, ddof=1)) )
fp.write('\n')

print('EAR_Fd')
print(repr(EAR_Fd_MM[0]))
print( str(np.std(EAR_Fd_M, ddof=1)))

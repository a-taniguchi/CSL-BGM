# -*- coding: utf-8 -*-
#action and attention (4 separate GMM+LDA)
#python ./learning/action.py '$folder' '$bun' '$trial' '$action
#Akira Taniguchi 2016/06/22-2017/04/12-2017/06/07
...
for c in list(itertools.product(za_candidate, Ad_candidate)):
      ...
      for zck in xrange(Kc):
        ...
        for zpk in xrange(Kp):
          ...
          for n in xrange(N):
            temp = temp * theta[zpk + dict["p"]][W_list.index(w_dn[n])]
          temp_w = temp_w + temp
          #temp_w = theta[zpk + dict["p"]][W_list.index(w_dn[n])]
          temp_pw = temp_pw + multivariate_normal.pdf(p_dm[c[1]], mean=Mu_p[zpk], cov=Sig_p[zpk]) * pi_p[zpk] * temp_w
          #print temp_pw,temp_w
        temp = 1e-1#00#1.0
        for n in xrange(N):
            temp = temp * theta[zok + dict["o"]][W_list.index(w_dn[n])]
        #temp_pw = temp_pw + temp
        temp_cpw = temp_cpw + multivariate_normal.pdf(c_dm[c[1]], mean=Mu_c[zck], cov=Sig_c[zck]) * pi_c[zck] * temp_pw *temp
      temp = 1e-1#00#1.0
      for n in xrange(N):
        temp = temp * theta[zok + dict["c"]][W_list.index(w_dn[n])]
      #temp_cpw = temp_cpw + temp
      #temp_ocpw = temp_ocpw + multivariate_normal.pdf(o_dm[c[1]], mean=Mu_o[zok], cov=Sig_o[zok]) * pi_o[zok] * temp_cpw
      temp_ocpw = temp_ocpw + exp(multivariate_normal.logpdf(o_dm[c[1]], mean=Mu_o[zok], cov=Sig_o[zok]) - max_log + log(pi_o[zok]) + log(temp_cpw) + log(temp))
      #print multivariate_normal.pdf(o_dm[c[1]], mean=Mu_o[zok], cov=Sig_o[zok]) * pi_o[zok]
    CDP[c[0]][c[1]] = CDP[c[0]][c[1]] * temp_ocpw * theta[c[0]           ][W_list.index(w_dn[n])]
    ...
    for m2 in mnotAd:
      ...
      temp = temp * temp_o * temp_p * temp_c
    CDP[c[0]][c[1]] = CDP[c[0]][c[1]] * temp

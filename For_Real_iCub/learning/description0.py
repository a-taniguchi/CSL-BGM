...
for m in xrange(M):
  ...
  if DATAnum == 0:
    ...
    for line in open(filename + 'image/object_RGB_BoF_'+str(D)+'_'+str(m+1).zfill(2)+'.csv', 'r'):
      itemList = line[:-1].split(',')
      for i in xrange(len(itemList)):
        if itemList[i] != '':
          #print c,i,itemList[i]
...
for w in range(len(W_list)):
  ...
  for c in za_candidate:
    CDPa[w] += multivariate_normal.pdf(a_d, mean=Mu_a[c], cov=Sig_a[c]) * pi_a[c] * theta[c][w]
  for c in zp_candidate:
    CDPp[w] += multivariate_normal.pdf(p_dm[Ad], mean=Mu_p[c], cov=Sig_p[c]) * pi_p[c] * theta[c + dict["p"]][w]
  for c in zp_candidate:
    CDPo[w] += multivariate_normal.pdf(o_dm[Ad], mean=Mu_o[c], cov=Sig_o[c]) * pi_o[c] * theta[c + dict["o"]][w]
  for c in zp_candidate:
...
####ファイル出力
fp = open(filename + 'argmax_w_dn_'+ learningname +'.csv', 'w')

...
#Akira Taniguchi 2016/06/22-2017/06/07
...
trialname = "ts" #param[1] #"testss"#raw_input("trialname?(folder) >")
foldername = datafolder + "ts(001-025)" #param[2] #datafolder + trialname+"("+str(sn).zfill(3)+"-"+str(en).zfill(3)+")"
#start = "1"#raw_input("start number?>")
#end   = "20"#raw_input("end number?>")
learningname = raw_input("learning trial name?>") #param[3]#"001"#
actionname = raw_input("action folder name?>") #param[4]

#sn = int(start)
#en = int(end)
Data = D #int(en) - int(sn) +1

#filename = foldername + '/' + learningname + '/' + actionname + '/'
filename = foldername + '/cnnpca006/' + actionname + '/'
...
CDP = [[pi_a[k]*1.0 for m in xrange(M)] for k in xrange(Ka)] #candidate propbability
...
for c in list(itertools.product(za_candidate, Ad_candidate)):
      ...
      for zck in xrange(Kc):
        ...
        for zpk in xrange(Kp):
          ...
          for F_temp in itertools.permutations(modality,N):
            ...
            temp = (1e-100)*float(M)#1.0
            for n in xrange(N):
              #print i,n,N[d],M[d]
              if F_temp[n] == "a":
                temp = temp * theta[c[0]           ][W_list.index(w_dn[n])] #* float(100**10)
              if F_temp[n] == "p":
                temp = temp * theta[zpk + dict["p"]][W_list.index(w_dn[n])] #* float(100**10)
              if F_temp[n] == "o":
                temp = temp * theta[zok + dict["o"]][W_list.index(w_dn[n])] #* float(100**10)
              if F_temp[n] == "c":
                temp = temp * theta[zck + dict["c"]][W_list.index(w_dn[n])] #* float(100**10)
            temp_w = temp_w + temp
            #print temp,temp_w
          temp_pw = temp_pw + multivariate_normal.pdf(p_dm[c[1]], mean=Mu_p[zpk], cov=Sig_p[zpk]) * pi_p[zpk] * temp_w #* float(1e+10)
          #print temp_pw,temp_w
        temp_cpw = temp_cpw + multivariate_normal.pdf(c_dm[c[1]], mean=Mu_c[zck], cov=Sig_c[zck]) * pi_c[zck] * temp_pw #* float(1e+10)
      temp_ocpw = temp_ocpw + multivariate_normal.pdf(o_dm[c[1]], mean=Mu_o[zok], cov=Sig_o[zok]) * pi_o[zok] * temp_cpw #* float(1e+10)
      #print temp_ocpw
      #print multivariate_normal.pdf(o_dm[c[1]], mean=Mu_o[zok], cov=Sig_o[zok]) * pi_o[zok]
    CDP[c[0]][c[1]] = CDP[c[0]][c[1]] * temp_ocpw #* float(1e+10)
    print CDP[c[0]][c[1]]
    ...
    for m2 in mnotAd:
      temp_o = 0.0
      #log - logmax
      #log_gauss = [multivariate_normal.logpdf(o_dm[m2], mean=Mu_o[zok], cov=Sig_o[zok]) for zok in xrange(Ko)]
      #gauss = [multivariate_normal.pdf(o_dm[m2], mean=Mu_o[zok], cov=Sig_o[zok]) for zok in xrange(Ko)]
      for zok in xrange(Ko):
        #temp_o = temp_o + exp(multivariate_normal.logpdf(o_dm[m2], mean=Mu_o[zok], cov=Sig_o[zok])-max(log_gauss)) * pi_o[zok]
        temp_o = temp_o + multivariate_normal.pdf(o_dm[m2], mean=Mu_o[zok], cov=Sig_o[zok]) * pi_o[zok]
        #print multivariate_normal.logpdf(o_dm[m2], mean=Mu_o[zok], cov=Sig_o[zok])+(10.0**4)*4
      temp_p = 0.0
      for zpk in xrange(Kp):
...
fp = open(foldername +'/' + learningname + '/' + actionname + '_candidate.csv', 'w')
#fp.write('theta\n')
for k in xrange(Ka):
  for m in xrange(M):
...
####ファイル出力
fp = open(foldername +'/' + learningname + '/' + actionname + '_argmax_ZAAD.csv', 'w')
fp.write(repr(argmax_zak) + ',' + repr(argmax_Ad) + '\n')
#fp.write('\n')
fp.close()

fp = open(foldername +'/' + learningname + '/' + actionname + '_argmax_a_d.csv', 'w')

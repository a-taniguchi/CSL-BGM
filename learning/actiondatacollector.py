...
def data_collect(trialname,sn,en,Data):
    ...
    for d in xrange(Data):
      ...
      #skin読み込み
      for line in open(foldername+str(d+sn).zfill(3)+'/skin/right_hand_comp/data.log','r'):
        itemList = line[:-1].split(' ')
        #if datatime >= int(itemList[1]):
        right_hand_comp = []
        for i in range(5*12):
          right_hand_comp = right_hand_comp + [float(itemList[i+2])]
        for i in range(4*12):
          right_hand_comp = right_hand_comp + [float(itemList[(i+2) + 8*12])]
      print "right_hand_comp",len(right_hand_comp),right_hand_comp
      fp.write("right_hand_comp,"+ repr(len(right_hand_comp))+"\n")
      for i in range(len(right_hand_comp)):
        fp.write(repr(right_hand_comp[i])+",")
      fp.write("\n\n")
      
      fp.write( repr(len(head)+len(left_arm)+len(right_arm)+len(torso)+len(right_hand_comp)) )

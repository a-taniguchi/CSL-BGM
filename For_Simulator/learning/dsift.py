# -*- coding: utf-8 -*-
#画像からDense SIFTを抽出し、k-meansでBoF化するプログラム
...
for trial in range(Data):
  ...
  for object in range(object_num):
    ...
    step_size = 2
    kp = np.array([cv2.KeyPoint(x, y, step_size) for y in range(0, gray.shape[0], step_size) 
                                        for x in range(0, gray.shape[1], step_size)])
 
    #kp, dsc = sift.detectAndCompute(gray,kp)
    ...
    #cv2.imshow("img", img)
    imgname = filename + 'image/dsift_0' + repr(object+1) + '.png'
...
fp = open(foldername+"("+str(sn).zfill(3)+"-"+str(en).zfill(3)+")"+'/DSIFT_kmeans.csv','w')
...
fp.close()
"""
fp = open(foldername+"("+str(sn).zfill(3)+"-"+str(en).zfill(3)+")"+'/RGB_kmeans.csv','w')
...
for trial in range(Data):
  ...
  for object in range(object_num):
    fp  = open(filename+'image/object_DSIFT_BoF_'+str(object+1).zfill(2)+'.csv','w')

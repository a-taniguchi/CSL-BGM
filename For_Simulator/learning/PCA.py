#! /usr/bin/env python
# -*- coding: utf-8 -*-
#CNN特徴量4096次元をPCAで次元圧縮するプログラム
#http://kensuke-mi.xyz/kensuke-mi_diary/2014/09/pca.html
#データはnumpy.array
#Akira Taniguchi 2016/07/24-
...
for trial in range(Data):
  ...
  for object in range(object_num):
    fp = open(filename+'image/object_CNN_PCA_'+str(object+1).zfill(2)+'.csv','w')
    for j in xrange(dim):
...
#Write PCA data
fp = open(foldername+"("+str(sn).zfill(3)+"-"+str(en).zfill(3)+")"+'/PCA.csv','w')
fp.write(repr(min)+'\n')
fp.write(repr(max)+'\n')

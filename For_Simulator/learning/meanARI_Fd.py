...
trialname = "testss"
trial_se = "(001-020)"#raw_input("trialname?(folder) >")
...
for s in range(Data):
  ...
  for line in open(foldername +'/' + filename + '/' + trialname + '_'+ filename +'_ARI_Fd.csv', 'r'):
    itemList = line[:-1].split(',')
    if (i != 0) and (itemList[0] != ''):
      #print i,itemList
      #MI_M[s] = MI_M[s] + [float(itemList[0])]
      ARI_Fd_M[s] = ARI_Fd_M[s] + [float(itemList[0])]
  ...
  ARI_Fd_M[s] = np.array(ARI_Fd_M[s])
...
fp.write('ARI_Fd\n')
fp.write(repr(ARI_Fd_MM[0])+'\n')
fp.write( str(np.std(ARI_Fd_M, ddof=1)) )
fp.write('\n')

print('ARI_Fd')
print(repr(ARI_Fd_MM[0]))
print( str(np.std(ARI_Fd_M, ddof=1)))

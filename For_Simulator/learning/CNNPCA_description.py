...
#Akira Taniguchi 2016/07/25-2017/04/05-

#すべての試行データごとに同じ特徴量を出力するようになっていることに注意
#PCが違うとPCAの結果が異なる可能性がある（testssはX250）→たぶん同じ
#そもそもプログラムがなにかおかしい。
...
trialname = "testss" #param[1] #raw_input("trialname?(folder) >")
...
trialname2 = "d" #param[1] #raw_input("trialname?(folder) >")
Data2 = int(raw_input("end number?(for new trial folder)>")) #
...
#####物体データを読み込み、CNN、RGB特徴を得る
foldername2 = datafolder + trialname2

"""
for trial in range(Data2):
...
for trial in range(Data):
  filename = datafolder + trialname+str(trial+sn).zfill(3)+'/'
...
if DATAnum == 0:
  ...
  for line in open(foldername +'/PCA'+str(Data)+'.csv','r'):
    if i == 0:
      min = float(line)
      i = i + 1
    if i == 1:
...
if DATAnum == 0:
  ...
  for line in open(foldername +'/RGB_kmeans'+str(Data)+'.csv','r'):
    itemList = line[:-1].split(',')
    for i in range(len(itemList)):
      if i != 0 and itemList[i] != "":
        cluster_bgr[int(itemList[0])] = cluster_bgr[int(itemList[0])] + [float(itemList[i])]
...
for trial in range(Data2):
  ...
  for object in range(object_num):
    imgname = filename2 + 'image/object_' + str(object+1).zfill(2) + '.ppm' 
    print imgname  
    ...
    fp = open(filename2+'image/object_CNN_fc6_'+str(object+1).zfill(2)+'.csv','w')
    fp.write(','.join(map(str, feat)))
    fp.close()
    
    imgname_color = filename2 + 'image/obj_mask_' + str(object+1).zfill(2) + '.ppm' 
    print imgname_color  
  ...
  data2 = []
  for object in range(object_num):
    fp  = open(filename2+'image/object_' + str(Descriptor) + '_'+str(object+1).zfill(2)+'.csv','w')
    if DATAnum == 0:
      fp2 = open(filename2+'image/object_RGB_BoF_'+str(object+1).zfill(2)+'.csv','w')
    elif DATAnum == 1:
      fp2 = open(filename2+'image/object_RGB_BoF_'+str(Data)+"_"+str(object+1).zfill(2)+'.csv','w')
    #CNN特徴の読み込み

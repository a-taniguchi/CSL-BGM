# -*- coding: utf-8 -*-
#画像からSIFTとRGBを抽出し、k-meansでBoF化するプログラム(データ数ごとにファイル名が異なるバージョン)
...
#Akira Taniguchi 2016/06/13-2016/11/30-
...
print '[Note] k-means cluster data will be chainged. Please check current files.'
...
for trial in range(Data):
  ...
  for object in range(object_num):
    ...
    sift = cv2.xfeatures2d.SIFT_create(contrastThreshold = 0.04/4, edgeThreshold = 10, sigma = 1.6-0.8)
...
#add = bow.add(dsc)
bow_bgr = cv2.BOWKMeansTrainer(k_rgb)
cluster_bgr = bow_bgr.cluster(des_bgr)
#print add
...
fp = open(foldername+"("+str(sn).zfill(3)+"-"+str(en).zfill(3)+")"+'/SIFT_kmeans'+str(Data)+'.csv','w')
...
fp = open(foldername+"("+str(sn).zfill(3)+"-"+str(en).zfill(3)+")"+'/RGB_kmeans'+str(Data)+'.csv','w')
for i in range(len(cluster_bgr)):
  fp.write(repr(i)+',')
  for j in range(len(cluster_bgr[i])):
    fp.write(repr(cluster_bgr[i][j])+',')
  fp.write('\n')
fp.close()
...
for trial in range(Data):
  ...
  for object in range(object_num):
    fp  = open(filename+'image/object_SIFT_BoF_'+str(Data)+'_'+str(object+1).zfill(2)+'.csv','w')
    fp2 = open(filename+'image/object_RGB_BoF_'+str(Data)+'_'+str(object+1).zfill(2)+'.csv','w')
    #min = 0

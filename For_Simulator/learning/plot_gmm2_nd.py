...
zp = []
pi_p = [0.0 for i in xrange(Kp)]

#[0.017826621173443864,0.28554229470170217,0.041570976925928926,0.1265347852145472,0.52852532198437785]
...
#['b', 'g', 'm', 'r', 'c','#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00' ]
...
for k,(mean,covar,color) in enumerate(zip(Mu_p,Sig_p,color_iter)):
        ...
        angle = np.arctan(u[0] / u[1])
        angle = 180 * angle / np.pi  # convert to degrees
        ell = mpl.patches.Ellipse([mean[1],mean[0]], v[0]*90, v[1]*90,-90+180 + angle, color=color)
        ell.set_clip_box(splot.bbox)
        ell.set_alpha(0.5*2*pi_p[k])
...
plt.savefig(foldername +'/' + filename + '/' + trialname + '_'+ filename +'_position_data_plot_p2nd.eps', dpi=150)
plt.savefig(foldername +'/' + filename + '/' + trialname + '_'+ filename +'_position_data_plot_p2nd.png', dpi=150)

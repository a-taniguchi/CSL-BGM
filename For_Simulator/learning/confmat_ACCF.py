...
def plot_confusion_matrix(#cm, classes,
    ...
    #cnf_matrix
    labels = ['grasp', 'touch', 'reach', 'look-at', 'front', 'left', 'right', 'red', 'green', 'blue', 'ball', 'box', 'cup']#sorted(list(set(y_true)))
    print labels
...
#�S�P�Q���A�S�P�O�w�K���ʂɑ΂���
for d in xrange(1,12+1):
  filename = datafolder + "d" + str(d).zfill(3) + '/'
  for trial in xrange(1,10+1):
    #����f�[�^�̓ǂݍ���
    for line in open(filename + "argmax_w_dn_true.csv", 'r'):
      line = line.replace("\r", "")
      line = line.replace("lookat", "look-at")
...
EARmat = f1_score(y_true, y_pred, average=None)
...
labels = ['grasp', 'touch', 'reach', 'look-at', 'front', 'left', 'right', 'red', 'green', 'blue', 'ball', 'box', 'cup']
####�t�@�C���o��
fp = open(foldername+'/'+ learningname + '_confmap_ACCF1.csv', 'w')
